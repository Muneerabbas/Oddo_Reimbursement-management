...
const ExpenseForm = () => {
  ...
  // Auto-calculate the USD estimated conversion whenever amount or currency changes
  useEffect(() => {
  ...
  // Handle native form mapping
  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
  };

  // Fake drag/drop UX bindings
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
            ...
            <div>
              ...
              <div className="flex relative items-stretch">
                <input
                  ...
                  className="w-full pl-4 pr-4 py-2 border border-slate-300 rounded-l-lg border-r-0 focus:ring-2 focus:ring-primary focus:border-primary focus:z-10 focus:outline-none transition-colors disabled:bg-slate-50 disabled:text-slate-500"
                />
                ...
                <select
                  ...
                  onChange={handleChange}
                  disabled={isLoading || isOcrLoading}
                  className="bg-slate-50 border border-slate-300 text-slate-700 text-sm rounded-r-lg focus:ring-2 focus:ring-primary focus:border-primary focus:z-10 focus:outline-none transition-colors px-3 py-2 disabled:text-slate-400"
                >
              ...
              {/* Intelligent Conversion Preview */}
              {convertedPreview && (
          ...
          {/* Artificial Neural OCR Receipt Mount */}
          <div>
            <div className="flex items-center justify-between mb-2">
               <label className="block text-sm font-medium text-slate-700">
                  Receipt Upload
               </label>
               {/* Conditionally activate OCR mock button if file is present */}
               {fileDetails && (
                  <button
                    type="button"
            ...
            <div 
              ...
              onDrop={handleDrop}
              className={`relative w-full border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center transition-colors ${fileDetails ? 'border-primary bg-primary/5' : 'border-slate-300 bg-slate-50 hover:bg-slate-100 hover:border-slate-400'}`}
            >
